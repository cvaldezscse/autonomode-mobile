package com.cvaldezscse.autonomode.core.listeners;

import com.cvaldezscse.autonomode.core.annotations.AnnotationHelper;
import com.cvaldezscse.autonomode.core.annotations.TestSource;
import com.cvaldezscse.autonomode.core.annotations.TestSourceStep;
import com.cvaldezscse.autonomode.core.config.Configuration;
import com.cvaldezscse.autonomode.core.model.TestCase;
import com.cvaldezscse.autonomode.core.model.TestExecution;
import com.cvaldezscse.autonomode.core.model.TestPlan;
import com.cvaldezscse.autonomode.core.model.TestStep;
import com.cvaldezscse.autonomode.core.reporting.ReportEngine;
...
public class ReportingClassListener implements IClassListener, ITestListener, ISuiteListener {
    private static final Logger logger = LoggerFactory.getLogger(ReportingClassListener.class);
    ...
    public void onBeforeClass(ITestClass testClass) {
        ...
        logger.info("Starting test preparation for: " + c.getSimpleName(), logger);

        try {
            logger.info("Class has TestSource: " + c.isAnnotationPresent(TestSource.class), logger);
            logger.info("Class has JamaTest: " + c.isAnnotationPresent(JamaTest.class), logger);
            TestCase testCase = null;
            if (c.isAnnotationPresent(TestSource.class)) {
                logger.info("Creating test case from TestSource", logger);
                ...
                logger.info("Creating test case from JamaTest", logger);
                onBeforeJamaTest(testClass);
                return;
            } else if (!jamaConf.isJamaEnabled() && c.isAnnotationPresent(JsonTest.class)) {
                logger.info("Creating test case from JsonTest", logger);
            ...
            logger.info("======================= ->" + testCase.getGlobalId() + "<- =======================", logger);
            logger.info("Adding test case to test plan: " + testCase.getGlobalId(), logger);
            testPlan.addTest(testCase);
        } catch (Exception e) {
            logger.error("Error in onBeforeClass: " + e.getMessage(), logger);
            e.printStackTrace();
            throw e;
        }
    }
    ...
    public void onAfterClass(ITestClass testClass) {
        logger.info("ReportingClassListener -> onAfterClass method executed", logger);
        ...
        try {
            ...
            if (testSourceStep != null) {
                ...
                if (testStep != null) {
                    ...
                    logger.warn("No test step found for ID: " + testSourceStep.value(), logger);
            }
        } catch (Exception e) {
            logger.info("Error processing TestSource result: " + e.getMessage(), logger);
        }
    }
    ...
    private void pushTestResultsToDynamo() {
        if (!BAMBOO_BUILD_ID.equalsIgnoreCase("Local")) {
            try {
                ...
                logger.info("Results published in the SQA Dashboard", logger);
            } catch (Exception e) {
                logger.error(format("Error pushing test results to the SQA Dashboard: {%s}", e.getMessage()), logger);
            }
        } else
            logger.info("Local execution, no results will be published in the SQA Dashboard", logger);
    }
    ...
    private void updateJamaTestRunStatus(String testRunId, Map<String, String> results) {
        try {
            if (testRunId == null || testRunId.isEmpty()) {
                logger.warn("Cannot update Jama status: No test run ID provided in configuration", logger);
                return;
            }
            ...
            if (totalSteps == 0) {
                logger.warn("Jama test case has no steps. Skipping update.", logger);
                return;
            }
            ...
            testCount++;
            logger.info(">>>>> ✅ Jama status updated successfully for testRunId: " + testRunId, logger);

        } catch (Exception e) {
            logger.error(">>>>> ❌ Error updating Jama test run status: " + e.getMessage(), logger);
        }
    }

    public void onBeforeJamaTest(ITestClass testClass) {
        Class<?> c = testClass.getRealClass();
        logger.info("Jama Test Case Start: " + c.getSimpleName(), logger);
        JamaTest jamaTest = AnnotationHelper.getJamaTest(c);
        ...
        if (testCase == null) {
            logger.error("No test was found in jama for " + jamaTest.value(), logger);
    ...
    public void onBeforeJsonTest(ITestClass testClass) {
        Class<?> c = testClass.getRealClass();
        logger.info("CLASS WITH JSON STARTED " + c.getName(), logger);
        JsonTest jsonTest = AnnotationHelper.getJsonTest(c);
        TestCase testCase = JsonHelper.createTestCaseFromJsonDetails(jsonTest);
        if (testCase == null) {
            logger.error("No test found for " + jsonTest.value(), logger);
        ...
        try {
            ...
            if (jamaTestStep != null) {
                ...
                if (testStep != null) {
                    ...
                    logger.warn("No test step found for ID: " + jamaTestStep.value(), logger);
            }
        } catch (Exception e) {
            logger.error("Error processing Jama test result: " + e.getMessage(), logger);
        }
    }
